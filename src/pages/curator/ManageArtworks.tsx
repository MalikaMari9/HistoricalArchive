...
export const ManageArtworks = () => {
  ...
  const filteredArtworks = artworks.filter(artwork => {
    ...
    // Using avg >= 4 as “approved” is just a placeholder—replace with real status when backend provides it.
    const isApproved = (artwork.averageRating ?? 0) >= 4.0;
    const matchesStatus =
      statusFilter === 'all' ||
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-green-600">
              {artworks.filter(a => (a.averageRating ?? 0) >= 4).length}
            </div>
          </CardContent>
        </Card>
        <Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-foreground">
              {artworks.reduce((sum, a) => sum + (a.totalRatings ?? 0), 0)}
            </div>
          </CardContent>
        </Card>
      </div>
              ...
              <TableBody>
                {filteredArtworks.length > 0 ? (
                  filteredArtworks.map((art) => {
                    const isApproved = (art.averageRating ?? 0) >= 4;
                    ...
                    return (
                      <TableRow key={art._id}>
                        ...
                        <TableCell>
                          <Badge className={getStatusColor(isApproved)}>
