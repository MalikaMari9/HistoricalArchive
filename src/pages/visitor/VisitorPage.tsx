...
interface Artifact {
  _id: string; // Updated to match gallery.tsx
  title: string;
  artist: string;
  period: string;
  imageUrl: string;
...
const VisitorPage = () => {
  ...
  useEffect(() => {
    const fetchTopRatedArtifacts = async () => {
      try {
        ...
        // The backend now returns a list, so we can directly set the state
  ...
  return (
    <DashboardLayout userRole="visitor" user={user}>
          ...
          <div className="container mx-auto px-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
              ...
              {featuredArtifacts.map((artifact) => (
                <Card key={artifact._id} className="group overflow-hidden hover:shadow-elegant transition-all duration-300 hover:-translate-y-2">
